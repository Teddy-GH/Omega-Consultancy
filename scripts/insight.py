...
def plot_rating_vs_sentiment(df, output_dir):
    plt.figure(figsize=(10, 6))

    # Fix: Assign `hue='sentiment'` and disable legend
    sns.violinplot(
        x='sentiment',
        y='rating',
        data=df,
        hue='sentiment',       # added to avoid FutureWarning
        palette='Set2',
        legend=False           # suppress the duplicate legend
    )
    ...
    output_path = os.path.join(output_dir, 'rating_vs_sentiment.png')
    plt.savefig(output_path)
    plt.close()
    plt.figure(figsize=(10, 6))
    # Updated to set hue equal to 'rating' and suppress legend
    sns.violinplot(x='sentiment', y='rating', hue='sentiment', data=df, palette='Set2', legend=False)
    plt.title('Rating vs Sentiment Distribution')
    plt.xlabel('Sentiment')
    plt.ylabel('Rating')
    plt.tight_layout()
